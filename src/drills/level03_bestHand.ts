...
export function generateBestHandDrill(rng: Rng): Drill {
  ...
  return {
    ...
    options,
    correctIndex,
    explanation: `Your best five cards are ${handLabel(best.bestFive)} — a ${correct} (${CATEGORY_HINT[best.category]}). Remember: you pick the best 5 of the 7 available. 🇰🇷 ${ko(HAND_KOREAN[best.category])}`,
  };
}
